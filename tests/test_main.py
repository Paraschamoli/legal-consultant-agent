...
async def test_handler_initialization():
    ...
    with (
        ...
        patch("legal_consultant_agent.main.initialize_agent", new_callable=AsyncMock) as mock_init,
        patch("legal_consultant_agent.main.run_agent", new_callable=AsyncMock, return_value=mock_response) as mock_run,
        patch("legal_consultant_agent.main._init_lock", new_callable=MagicMock()) as mock_lock,
    ):
...
@pytest.mark.asyncio
async def test_handler_with_legal_query():
    """Test that handler can process a legal education query."""
    messages = [
        {
            "role": "user",
            "content": "What are the basic elements of a valid contract?",
        }
    ]

    mock_response = MagicMock()
    mock_response.run_id = "legal-run-id"
    mock_response.content = "Legal information provided with disclaimer."
    ...
    assert result is not None
    assert result.run_id == "legal-run-id"
    assert result.content == "Legal information provided with disclaimer."
